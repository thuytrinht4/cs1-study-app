...
- `cs1/schedule.py` — the fixed dated study map (15 Jun → 22 Sep 2026): all 15 CS1 topics learned by 9 Aug (high-yield ⭐ weighted & recurring), then past papers/mocks/R, CS1A taper, and a CS1B (R) gap. Drives "today's focus" on Home, the full map on the Plan page, and the "today's topics" exam deck.
- `cs1/plan.py` — pure planning logic on top of FSRS: adaptive new-card target, coverage tracking, behind-ness, forecast (the spacing layer under the schedule map).
- In Deep (and revealed Fast) mode you can **ask Claude follow-up questions** about the question, your answer, or the concept — a mini tutor chat per card (`ai.followup_answer`, cheap Haiku).
- `data/exam_cards*.json` — 72 past-paper-style, multi-part exam questions (every module has ≥8, so the schedule's daily topic always has enough) with full model answers + mark schemes, weighted to GLMs, regression, hypothesis testing and Bayesian. Import/update from Home; study via the "Exam-style questions" deck or **"📝 Exam Qs — today's topics"** (auto-filtered to the module(s) you're revising today); mark schemes also sharpen the AI marking.
